...
void CollisionListener::BeginContact(b2Contact * contact)
{
	...
	*/
	...
	PoseidonsTrident(bodyUserData, bodyUserData2, fixtureA, fixtureB); //check for the wave generated by the staff colliding
...
void CollisionListener::PoseidonsTrident(void * userData1, void * userData2, b2Fixture * fixture1, b2Fixture * fixture2)
{
	if (fixture1->GetFilterData().categoryBits == ITEM && fixture2->GetFilterData().categoryBits == ENEMY) {
		//check if actually staff and then push and deal damage
		if (userData1 == "PoseidonsStaff") { // make sure it was the staff
			
			static_cast<GameCharacters*>(userData2)->dynamicBody->SetTransform(static_cast<PoseidonsStaff*>(fixture1->GetUserData())->Box2DBody->GetPosition(),0); // move along with the wave
			static_cast<GameCharacters*>(userData2)->health = static_cast<GameCharacters*>(userData2)->health - static_cast<PoseidonsStaff*>(fixture1->GetUserData())->itemDamage; // deal damage as it carries the enemy along
			

		}
	}
	else if (fixture1->GetFilterData().categoryBits == ENEMY && fixture2->GetFilterData().categoryBits == ITEM) {
		if (userData2 == "PoseidonsStaff") { // make sure it was the staff

			static_cast<GameCharacters*>(userData1)->dynamicBody->SetTransform(static_cast<PoseidonsStaff*>(fixture2->GetUserData())->Box2DBody->GetPosition(), 0); // move along with the wave
			static_cast<GameCharacters*>(userData1)->health = static_cast<GameCharacters*>(userData1)->health - static_cast<PoseidonsStaff*>(fixture2->GetUserData())->itemDamage; // deal damage as it carries the enemy along
